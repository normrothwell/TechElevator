 ...
 ***************************************************************************************************/
	...
	public void dealHand(CardDeck aDeck) {             // Deal a Hand
		for (int i=0; i < NUMBERCARDSINHAND; i++ ) {   // For the number of cards in hand
			super.hand.add(aDeck.dealCard());         //     Add a card from deck passed to hand
		}			
	}
...
//		this.sortByValue();
		...
		if (((this.hand.get(0).getValue() == PlayingCard.CardValue.ONE)
		   &&(this.hand.get(4).getValue() == PlayingCard.CardValue.KING)
		   &&(this.hand.get(3).getValue() == PlayingCard.CardValue.QUEEN)
		   &&(this.hand.get(2).getValue() == PlayingCard.CardValue.JACK)
		   &&(this.hand.get(1).getValue() == PlayingCard.CardValue.TEN))
		   && hasFlush())
		  {	
