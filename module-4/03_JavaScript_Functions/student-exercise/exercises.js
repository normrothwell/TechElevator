 ...
 */
function isSummer(month, day = 1, hemisphere = 'northern') { //takes it a month, a default day of 1 and a default hemisphere of one and returns if it is or is not summer
  if (month === 7 || month === 8) {
 ...
 */
 ...
 function isAdmitted(gpa, satScore, recommendation){
   ...
   if ((gpa > 3 && recommendation == true) || (satScore > 1200 && recommendation == true)){
     admitted = true;
   }
   return admitted;
 }
 ...
 */
 ...
 let answer = wordArray.map((word) => 'Happy ' + word);
 return answer;
 }
 ...
 */
 ...
 function getFullAddressesOfProperties(myArray){
   return myArray.map((house) => {return [house.streetNumber, house.streetName, house.streetType, house.city, house.state, house.zip].join(' ')});
 }
 ...
 */
 ...
 function findLargest(numArray){
   ...
   numArray.forEach((element) => {if (element >= answer) {answer = element}});
    return answer;
 }
