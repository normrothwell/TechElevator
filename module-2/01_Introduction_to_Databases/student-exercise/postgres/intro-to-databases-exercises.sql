...
SELECT name, continent, population, gnp, lifeexpectancy FROM country WHERE lifeexpectancy is not null AND lifeexpectancy > 80;

-- 9. The name and population of all cities in the USA with a population of greater than 1 million people (9 rows)

SELECT name, population FROM city WHERE population > 1000000 AND population is not null AND countrycode = 'USA';

-- 10. The name and population of all cities in China with a population of greater than 1 million people (35 rows)

SELECT name, population FROM city WHERE population is not null AND population > 1000000 AND countrycode = 'CHN';
...
SELECT name, region FROM country WHERE continent = 'North America' OR continent = 'South America' EXCEPT SELECT name, region FROM country WHERE region = 'Caribbean';
...
SELECT (gnp/population) AS "Per capita GNP" FROM country WHERE continent = 'Europe';

-- 22. The number of years since independence for all countries that have a year of independence (192 rows)

SELECT (2020-indepyear) AS "Years since indepencdence" FROM country WHERE indepyear is not null;