...
<div>
	...
	<ul id = "forecast-data">
		...
		<li>High: <fmt:formatNumber type = "number" maxFractionDigits = "2" value = "${(day.lowTemp - 32) * (5/9)}"/> C</li>
		<li>Low: <fmt:formatNumber type = "number" maxFractionDigits = "2" value = "${(day.highTemp - 32) * (5/9)}"/> C</li>
		</c:when>
		<c:otherwise>
		<li>High: <c:out value = "${day.highTemp}"/> F</li>
		<li>Low: <c:out value = "${day.lowTemp}"/> F</li>
		</c:otherwise>
		</c:choose>
		<br>
