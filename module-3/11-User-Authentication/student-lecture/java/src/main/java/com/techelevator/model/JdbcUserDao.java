...
public class JdbcUserDao implements UserDao {
    ...
    public User saveUser(String userName, String password, String role) {
    	...
    	String query = "INSERT INTO users (username, password, salt, role) VALUES (?, ?, ?, ?) RETURNING id";
    	long newId = jdbcTemplate.queryForObject(query, Long.class, userName, saltedPair.getHashedPassword(), saltedPair.getHash(), role);
    ...
    public User getValidUserWithPassword(String userName, String password) {
        ...
        SqlRowSet query = jdbcTemplate.queryForRowSet(sqlSearchForUser, userName.toUpperCase());
        
        while(query.next()) {
        	
        	String storedSalt = query.getString("salt");
        	String storedPassword = query.getString("password");
        	String hashedPassword = passwordHasher.computeHash(password, Base64.decode(storedSalt));
        	
        	if (storedPassword.equals(hashedPassword)) {
        		return mapResultToUser(query);
        	}
